...
use app\common\logic\MajorMember;
use think\console\Command;
use think\console\Input;
use think\console\Output;
use app\common\logic\PlatformSetting as PlatformSettingLogic;
use think\Db;
 ...
 * 每天03:00定时执行
 */
class BetweenReward extends Command
{
    protected function configure()
    {
        $this->setName('between_reward')
...
//                    dump($total);
                        foreach ($superiors as $key => $item){
                            ...
                            if(!$is_send) {
                                ...
                                if($commission_money > 0 && $key>0 &&$real_gold >= 0){
                                    $data = [];
